<template>
    <div id="drag-drop-area" class="fixed z-10 inset-0 overflow-y-auto" v-if="showEditModal" @dragover="dragover" @dragleave="dragleave" @drop="drop">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            <div class="mt-2">
                                <div class="mt-4">
                                    ...
                                    <input aria-label="Name" id="voice-media" name="name" type="file" required class="h-0 overflow-hidden" ref="file" placeholder="Name" @change="updateMediaFile($event)" >
                                ...
                                <div class="mt-4">
                                    <select aria-label="Accent" name="accent" type="text" required class="appearance-none rounded relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:border-blue-300 focus:z-10 sm:text-sm sm:leading-5" @change="updateAccent($event)">
                                        ...
                                        <option :value="a.id_accent" :selected="currentMedia.accent === a.accent" v-for="a in accents">{{a.accent}}</option>
                                    </select>
                                </div>
                                <div class="mt-4">
                                    <select aria-label="Language" name="language" type="text" required class="appearance-none rounded relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:border-blue-300 focus:z-10 sm:text-sm sm:leading-5" @change="updateLanguage($event)">
                                        ...
                                        <option :value="l.id_language" :selected="currentMedia.language === l.language" v-for="l in languages">{{l.language}}</option>
                                    </select>
                                </div>
                                <div class="mt-4">
                                    <select aria-label="Platform" name="platform" type="text" required class="appearance-none rounded relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:border-blue-300 focus:z-10 sm:text-sm sm:leading-5" @change="updatePlatform($event)">
                                        ...
                                        <option :value="p.id_platform" :selected="currentMedia.platform === p.platform" v-for="p in platforms">{{p.platform}}</option>
                                    </select>
                                </div>
                                <div class="mt-4">
                                    <select aria-label="Style" name="style" type="text" required class="appearance-none rounded relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:border-blue-300 focus:z-10 sm:text-sm sm:leading-5" @change="updateStyle($event)">
                                        ...
                                        <option :value="s.id_style" :selected="currentMedia.style === s.style" v-for="s in styles">{{s.style}}</option>
                                    </select>
                                </div>
                                <div class="mt-4">
                                    <select aria-label="Tone" name="tone" type="text" required class="appearance-none rounded relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:border-blue-300 focus:z-10 sm:text-sm sm:leading-5" @change="updateTone($event)">
                                        ...
                                        <option :value="t.id_tone" :selected="currentMedia.tone === t.tone" v-for="t in tones">{{t.tone}}</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-white px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <span class="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto">
                        <button type="button" class="inline-flex justify-center w-full rounded-md border border-transparent px-4 py-2 bg-blue-600 text-base leading-6 font-medium text-white shadow-sm hover:bg-blue-500 focus:outline-none focus:border-blue-700 focus:shadow-outline-blue transition ease-in-out duration-150 sm:text-sm sm:leading-5" @click="updateMedia()">
                        Save Media
                        </button>
                    </span>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters('VoiceMediaList', ['showEditModal', 'currentMedia', 'accents', 'languages', 'platforms', 'styles', 'tones']),
        },
        methods: {
            ...mapActions('VoiceMediaList', ['setModalVisibility', 'fetchAccentData', 'fetchLanguageData', 'fetchPlatformData', 'fetchStyleData', 'fetchToneData', 'updateMedia', 'setMedia', 'setName', 'setAccent', 'setLanguage', 'setPlatform', 'setStyle', 'setTone']),
            ...
            drop(event) {
                ...
                this.updateMediaFile(); // Trigger the onChange event manually
                // Clean up
                event.currentTarget.classList.remove('bg-green-300');
            }
        }
